#pragma once
#ifndef  __FightGame__PickUpBarBuilder__
#define __FightGame__PickUpBarBuilder__
#include"TileBuilder.h"

class PickUpBarBuilder :
	public TileBuilder
{
private:
public:
	Tile* getResult(Vec2 pos) {
		...
		this->_texture = new Texture("TableCyBar.tga", Vec4(256, 256, 5, 1));
		Animator *player = new Animator();
		this->_animatorArray->push_back(player);

		Animation2D *a = new Animation2D(Vec4(0, 256, 256, 256), 30);
		this->_animatorArray->at(0)->_animation2DArray->push_back(a);
		Animation2D *b = new Animation2D(Vec4(0, 256*2, 256, 256), 200);
		...
		Tile *_tile = new PickUpBar	(m_sprite);
		return _tile;
	}
